...
cat > "$CLAUDE_DIR/settings.json" << 'SETTINGS'
{
  ...
  "permissions": {
    "allow": [
      "Bash(rg *)", "Bash(fd *)", "Bash(bat *)", "Bash(eza *)", "Bash(sd *)",
      "Bash(jq *)", "Bash(yq *)", "Bash(xsv *)", "Bash(miller *)", "Bash(mlr *)",
      "Bash(htmlq *)", "Bash(choose *)", "Bash(duckdb *)", "Bash(dust *)",
      "Bash(tokei *)", "Bash(procs *)", "Bash(xh *)", "Bash(doggo *)",
      "Bash(git log *)", "Bash(git diff *)", "Bash(git status *)",
      "Bash(git branch *)", "Bash(git add *)", "Bash(git commit *)",
      "Bash(git checkout *)", "Bash(git switch *)", "Bash(git stash *)",
      "Bash(gh *)", "Bash(lazygit *)", "Bash(onefetch *)",
      "Bash(gitleaks *)", "Bash(git-cliff *)", "Bash(difftastic *)",
      ...
      "Bash(pwd)", "Bash(which *)", "Bash(type *)", "Bash(echo *)", "Bash(date *)",
      "Bash(python *)", "Bash(uv *)", "Bash(bun *)",
      "Bash(pytest *)", "Bash(python -m pytest *)",
      "Bash(docker ps *)", "Bash(docker logs *)", "Bash(docker compose *)",
      "Bash(docker build *)", "Bash(docker images *)", "Bash(docker run *)",
      "Bash(docker exec *)", "Bash(docker inspect *)",
      "Bash(kubectl get *)", "Bash(kubectl describe *)", "Bash(kubectl logs *)",
      "Bash(terraform plan *)", "Bash(terraform validate *)", "Bash(terraform fmt *)",
      "Bash(tflint *)", "Bash(infracost *)", "Bash(ansible-lint *)",
      "Bash(trivy *)", "Bash(nuclei *)", "Bash(nmap *)", "Bash(osv-scanner *)",
      "Bash(hcloud *)", "Bash(aws *)",
      "Bash(pandoc *)", "Bash(glow *)", "Bash(tldr *)",
      "Bash(hyperfine *)", "Bash(pueue *)", "Bash(watchexec *)",
      "Bash(compgen *)",
      "Bash(* --help)", "Bash(* -h)", "Bash(* --version)",
      "Bash(uv tool list *)", "Bash(uv tool install *)",
      "Bash(cargo install *)", "Bash(bun install *)",
      "Read(*)", "Edit(*)", "MultiEdit(*)", "Write(*)",
      "Glob(*)", "Grep(*)", "Skill(*)"
    ],
    "deny": [
      "Bash(rm -rf *)", "Bash(rm -r /*)", "Bash(rm -r *)",
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "jq -r '.tool_input.command // empty' | { read -r -d '' cmd || true; case \"$cmd\" in *'rm -rf'*|*'rm -r '*|*'rm --recursive'*) echo 'Use trash-put instead of rm -r' >&2; exit 2;; *'git push'*'--force'*|*'git push -f'*) echo 'Force push blocked. Use a feature branch.' >&2; exit 2;; *'pip install'*|*'sudo pip'*|*'npm install -g'*) echo 'Use uv tool install (Python) or bun install -g (JS) instead.' >&2; exit 2;; *'git commit'*) branch=$(git branch --show-current 2>/dev/null); if [ \"$branch\" = main ] || [ \"$branch\" = master ]; then echo \"Cannot commit on $branch. Create a feature branch first.\" >&2; exit 2; fi;; esac; exit 0; }",
            "timeout": 5
          }
        ]
      },
...
if [ ! -d ~/.claude/skills/tdd ]; then
  ...
  if [ -d /tmp/superpowers/skills ]; then
    ...
    cp -r /tmp/superpowers/skills/root-cause-tracing ~/.claude/skills/root-cause-tracing 2>/dev/null || true
    cp -r /tmp/superpowers/skills/defense-in-depth ~/.claude/skills/defense-in-depth 2>/dev/null || true
