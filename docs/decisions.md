...
**Consequences**: Password rotation is idempotent. Re-runs always leave container, credentials, and settings.json in agreement. Design intent: the **container is the follower** — when a mismatch is detected, the container is restarted to adopt the new credentials (not the reverse). `~/.config/letta/credentials` and `settings.json` are always written first; the container catches up on restart. Trade-off: one extra `docker inspect` call per letta setup (negligible). Requires docker CLI available, which is already a hard dependency for letta service setup.

## ADR-037: claude-subconscious hook exit codes — non-blocking on Letta API errors (2026-03-23)
**Status**: Accepted
**Context**: Two claude-subconscious hook scripts exit with code 1 on Letta API errors: `session_start.ts` (Letta 401/404 on session notify) and `sync_letta_memory.ts` (missing key, 401/404 on memory sync). CC treats any non-zero hook exit as a hook error, surfacing "SessionStart:startup hook error" and "UserPromptSubmit hook error" at every prompt — even when Letta is temporarily unavailable or misconfigured. The `/dev/tty` crash (ADR-035) was the first of this class; these are two more. All three are in the same plugin and will be wiped by `claude plugin update claude-subconscious`. The exit-code patches must therefore live in titan's plugin-patch phase (lib/12-plugins.sh) alongside the `/dev/tty` patch, not only on the live machine.
**Decision**: Add two Python-inline patches to the subconscious plugin-patch block in `lib/12-plugins.sh`: (1) `session_start.ts` — replace the final `process.exit(1)` (Letta error handler) with `process.exit(0)` + non-blocking comment. (2) `sync_letta_memory.ts` — replace both `exit(1)` sites (missing key + caught error) with `exit(0)`. Patches are guarded by `grep -q 'non-blocking.*Letta errors'` so they are idempotent across re-runs. Patterns are fragile if plugin is updated — `warn` on mismatch rather than fail.
**Consequences**: Letta errors become silent non-blocking failures. CC session start and prompt submit no longer error when Letta is unavailable. Subconscious whispers when Letta is healthy, silently absent when not. Trade-off: errors are less visible — check `~/.claude/logs/failures.jsonl` or `/tmp/letta-claude-sync-*/session_start.log` to diagnose Letta issues. Three regression tests added (CANON prefix).

## ADR-038: n8n native install via npm (supersedes ADR-025) (2026-03-23)
**Status**: Accepted
**Context**: n8n ran as Docker container, pinned to 2.10.4 on ARM64 due to isolated-vm segfault on Alpine/musl (ADR-025). The upstream fix shipped in n8n PR #26765 (backported to 2.11.x/2.12.x). Docker adds ~25 MiB overhead with no isolation benefit for n8n — it's a single-process Node.js app. mise already manages Node 22.x. The ARM64 pin and Type=oneshot+docker-in-systemd pattern (ADR-026) are unnecessary complexity for n8n alone.
**Decision**: Install n8n globally via `npm install -g n8n` using mise-managed Node. systemd unit changes from `Type=oneshot` + `docker run -d` to `Type=simple` + `n8n start`. Add `--n8n-skip` flag (added to `--minimal` too). On re-run, detect Docker-based unit and migrate (stop container, remove, keep `~/.n8n` data intact). Remove ARM64 image pin entirely.
**Consequences**: `build-essential` + `python3` needed for native addon compilation (already in lib/03-packages.sh). Upgrades via `npm update -g n8n` instead of `docker pull`. ADR-025 superseded. ADR-026 still applies to letta/ollama/ccflare (they genuinely need Docker). Data in `~/.n8n` preserved across migration since it was already a volume mount at the same path.

## ADR-039: desktop-mode root XDG_RUNTIME_DIR fix (2026-03-23)
**Status**: Accepted
**Context**: `sudo bash titan-setup.sh --mode desktop` runs as root. PR #58 fixed `HOME` and `PATH` to point at the target user, but missed `XDG_RUNTIME_DIR`. Without it, `systemctl --user` targets `/run/user/0` (root's nonexistent persistent session). Docker-based services (letta, ccflare) survived because their containers run detached from systemd. Native `Type=simple` services (n8n, claudecodeui, letta-ctrl) started in root's session and died immediately. Discovered during n8n native migration live test.
**Decision**: In the desktop-root block in `lib/02-cli.sh`, also export `XDG_RUNTIME_DIR=/run/user/$TARGET_UID` and `DBUS_SESSION_BUS_ADDRESS=unix:path=/run/user/$TARGET_UID/bus`, and call `loginctl enable-linger` so the user session persists at boot. VPS mode is unaffected (re-exec as user handles it).
**Consequences**: All `systemctl --user` calls in desktop mode now target the correct user session. Native services (n8n, claudecodeui, letta-ctrl) start and stay running. `loginctl enable-linger` ensures they survive reboot without active login.
